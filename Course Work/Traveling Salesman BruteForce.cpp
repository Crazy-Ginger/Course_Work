...
//    MAIN is the main program for TSP_BRUTE.
{
	...
	cin >> distance_filename;

	r8mat_header_read(distance_filename, m, n); //Finds the size of the problem. 

	cout << "\n";
	cout << "  Number of cities is " << n << "\n";

	distance = r8mat_data_read(distance_filename, n, n); //creates a local array storing the distances between each location
	
	//  Print the distance matrix.
	r8mat_print(n, n, distance, "  Distance matrix:");

	//  Examine every permutation.
	total_max = -r8_huge(); //returns a big big number ??
	total_min = r8_huge();
	total_ave = 0.0;
	...
	for (; ; ) //infinite loop
	{
		perm_next3(n, p, more); //finds how many possible permuations there are of the set of nodes
		paths = paths + 1;
		total = 0.0;
		i1 = n - 1;
		for (i2 = 0; i2 < n; i2++)
...
//
//  Discussion:
//    The routine is initialized by calling with MORE = TRUE in which case it returns the identity permutation.
//    If the routine is called with MORE = FALSE then the successor of the input permutation is computed.
...
//    
//	  If MORE is TRUE, then P is assumed to contain the "previous" permutation, and on P(I) is the value of the I-th object under the next permutation.
...
//    Each line that is not ignored is assumed to contain exactly (or at least) M real numbers, representing the coordinates of a point.
...
//	  The program will stop reading data once N values have been read.
//    Output, double R8MAT_DATA_READ[M*N], the data.
{
	...
	if (!input) //error if the file failed to load
	{
		cerr << "\n";
	...
	while (j < n)
	{
		...
		if (line[0] == '#' || s_len_trim(line) == 0) //checks if the line is commented out or null
		{
			continue;
		}
...
//    Output, int &N, the number of points.
	...
	if (m <= 0) //error is the table is smaller than 1
	{
		cerr << "\n";
	...
	if (n <= 0) //error if there are no nodes in the table
	{
		cerr << "\n";
		cerr << "R8MAT_HEADER_READ - Fatal error!\n";
